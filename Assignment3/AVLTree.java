 ...
 ******************************************************
 * Assignemnt 3
 * Name: Irizarry_Ryan Cesar
 * UID: rci11
 ********************************************************
 * A class that implements an AVL Tree and its properties
 */
public class AVLTree<T extends Comparable<T>> {
  ...
  public int getBalance(AVLTreeNode<T> tree) {
    if (tree == null) {
      return 0;
    }
  ...
  private AVLTreeNode<T> minimum(AVLTreeNode<T> tree) {
    ...
    T min = tree.key;
    AVLTreeNode<T> minNode = tree;
    if (tree.left != null && (tree.left.key).compareTo(min) < 0) {
      minNode = minimum(tree.left);
    } else if (tree.right != null) {
      minNode = minimum(tree.right);
    }
    return minNode;
  }
  ...
  private AVLTreeNode<T> remove(AVLTreeNode<T> tree, AVLTreeNode<T> z) {
    ...
    if (cmp < 0) { // The node to be deleted is in the "left subtree of tree"
        ...
        if (tree.left != null && tree.right != null) {
            if (height(tree.left) > height(tree.right)) {
                AVLTreeNode<T> maxNode = maximum(tree.left);
                tree.key = maxNode.key;
                tree.left = remove(tree.left, maxNode);
            } else {
                AVLTreeNode<T> minNode = minimum(tree.right);
    ...
    if (balance > 1) {
        if (getBalance(tree.left) >= 0) {
            ...
            tree.left = leftLeftRotation(tree.left);
            return rightRightRotation(tree);
        }
    }
    if (balance < -1) {
        if (getBalance(tree.right) <= 0) {
            ...
            tree.right = rightRightRotation(tree.right);
            return leftLeftRotation(tree);
        }
    }
    return tree;
